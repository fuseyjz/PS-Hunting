...
<#
    .SYNOPSIS
        ...
        Enumerate through every process and retrieve the Named Pipes for analysis.

        This will exclude the named pipes listed in the $Whitelist.
    ...
    .EXAMPLE
        ...
        PS > Get-NamedPipes | ft

        Handle HandleFlags ProcID ProcName Type Name
        ------ ----------- ------ -------- ---- ----
        0x01E8 NONE          4452 beacon   File \Device\NamedPipe\msagent_65ec
        0x034C INHERIT       2636 rundll32 File \Device\NamedPipe\msagent_65ec
        0x0360 NONE          2636 rundll32 File \Device\NamedPipe\msagent_65ec
        0x034C INHERIT       7456 rundll32 File \Device\NamedPipe\msagent_65ec
        0x0354 INHERIT       7456 rundll32 File \Device\NamedPipe\msagent_65ec
        [...Snip...]
#>
    ...
    public static class HandleEnum {
        ...
        [DllImport("ntdll.dll")]
        public static extern UInt32 NtQueryObject(
            IntPtr Handle,
            UInt32 ObjectInformationClass,
...
'@
    ...
    $Whitelist = "\Device\NamedPipe\eventlog", "\Device\NamedPipe\wkssvc" `
        + "\Device\NamedPipe\vgauth-service", "\Device\NamedPipe\trkwks" `
        + "\Device\NamedPipe\epmapper", "\Device\NamedPipe\LSM_API_service" `
        + "\Device\NamedPipe\srvsvc", "\Device\NamedPipe\browser" `
        + "\Device\NamedPipe\atsvc", "\Device\NamedPipe\spoolss" `
        + "\Device\NamedPipe\MsFteWds", "\Device\NamedPipe\SearchTextHarvester" `
        + "\Device\NamedPipe\lsass"

    foreach($proc in (Get-Process)) {
        if($proc.Id -ne 0 -and $proc.Id -ne 4) {
                $Filter = (Get-Handles -ProcID $proc.Id) | Where-Object {$_.ObjectType -eq "File"} | Get-HandleObject
                $Filter2 = $Filter | Where-Object {$_.Name -match "NamedPipe"}
                $Output = $Filter2 | Where-Object {$Whitelist -notcontains $_.Name}
                Write-Output ($Output)
        }
    }
}

function Get-HandleObject {
<#
    .SYNOPSIS

        Built onto Ruben's Get-Handles to retrieve the Handle's type name 
        using NtQueryObject. This is built on the use-case specifically to retrieve 
        the "File" type handles.

        This function will accept the output from Get-Handles and 
        pipe into Get-HandleObject.
    ...
    .EXAMPLE
        ...
        PS > Get-Handles -ProcID 1048 | Where-Object {$_.ObjectType -eq "File"} | Get-HandleObject | ft
        
        Handle ProcID ProcName Type Name
        ------ ------ -------- ---- ----
        0x0044   1048 svchost  File \Device\HarddiskVolume1\Windows\System32
        0x0078   1048 svchost  File \Device\CNG
        0x0110   1048 svchost  File \Device\HarddiskVolume1\Windows\System32\en-US\svchost.exe.mui
        0x0220   1048 svchost  File \Device\KsecDD
        0x0278   1048 svchost  File \Device\NamedPipe\eventlog
        [...Snip...]
#>
    ...
    param(
     ...
     Process {
        $PHandle = [HandleEnum]::OpenProcess(
            [ProcessAccessFlags]::DupHandle, $True, $ID)

        $lpTargetHandle = [IntPtr]::Zero
        [bool]$Dupe = [HandleEnum]::DuplicateHandle(
            $PHandle, 
            $Handle, 
            [HandleEnum]::GetCurrentProcess(), 
            [ref]$lpTargetHandle, 0, $False, 
            0x00000002)

        [HandleEnum]::CloseHandle($PHandle) | Out-Null
       
        if($Dupe) {
            $ReturnLength = 0
            $ObjectLength = [Runtime.InteropServices.Marshal]::SizeOf([type][OBJECT_NAME_INFORMATION]) + 256
            [IntPtr]$ObjectInfo = [Runtime.InteropServices.Marshal]::AllocHGlobal($ObjectLength)
    
            $Query = [HandleEnum]::NtQueryObject(
                $lpTargetHandle, 1, 
                $ObjectInfo, 
                $ObjectLength, 
                [ref]$ReturnLength
            ...
            if ($Query -eq 0x00000000) {
                $Convert = [Runtime.InteropServices.Marshal]::PtrToStructure(
                    $ObjectInfo, [type][OBJECT_NAME_INFORMATION])

                $Name = $Convert.Name.ToString()
                $ProcName = ((Get-Process -Id $ID).ProcessName)

                if($Name) {
                    $HandleOut = "0x{0:X4}" -f [int]$Handle
                    $OutputDetail = New-Object PSObject
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name Handle -Value $HandleOut
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name HandleFlags -Value $HFlags
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name ProcID -Value $ID
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name ProcName -Value $ProcName
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name Type -Value $Type
                    $OutputDetail | Add-Member -MemberType Noteproperty -Name Name -Value $Name
                    Write-Output $OutputDetail          
                }
            }
            [Runtime.InteropServices.Marshal]::FreeHGlobal($ObjectInfo) | Out-Null
        }
        [HandleEnum]::CloseHandle($lpTargetHandle) | Out-Null
     }
}
